...
|---|---|
| **現在のフェーズ** | P7（仕上げ） |
| **次にやること** | **P11-6 の残り（AI が根拠に書いた数値がプロンプトに実在するかの照合）→ P11-7（実データ4パターンの通し確認）。** P11-1〜P11-5 と P11-6 の前半（`fundamental`・`data_scope_note`・レポートへの財務の反映）は完了。プロンプトは実測で **60日 28,420→15,355 トークン / 120日 約53,000→16,305 トークン**（財務を足したうえで減）。**P7-2（通しの動作確認）は後回し**（Gemini のモデル名と RPM/TPM/RPD をユーザーが設定タブへ入力しないと進められない。`gemini-2.5-flash` で動作確認済み） |
| **リポジトリ状態** | `main` に P12 までマージして push 済み。**P11 は `feature/p11-financials` ブランチで P11-4 まで実装済み（未マージ・未 push）**。コミットのメールアドレスはリポジトリ設定で GitHub の noreply アドレスにしてある |
...
| P11 | 財務数値の取り込みと指標（ユーザー要望） | WIP（P11-1〜P11-4 完了） |
...
| P11-6 | `WIP` | 出力スキーマとレポート、数値の実在検証 | SPEC §2.9.9。`fundamental` を `technical` と `disclosure` の間に、`data_scope_note` を末尾に追加。レポートに財務指標の表を出す（ここまで完了）。**残り: AI が根拠に書いた数値がプロンプトに実在するかを照合**し、結果をレポートに出す | P11-5 P6-5 | `ai/schema.py` `ai/report.py` `templates/` |
| P11-7 | `TODO` | 実データでの確認 | 日本基準・IFRS・単体のみ・財務なしの4パターンで、取り込み → 指標 → プロンプト → レポートを通す。トークン数の前後比較を記録する | P11-6 | — |
