...
|---|---|
| **現在のフェーズ** | P7（仕上げ） |
| **次にやること** | **P7-2（通しの動作確認）→ そのあと P11-1。** P7-2 は実機（`start.bat`）で 起動→登録→需給取得→開示取得→チャート→AI 分析→レポート閲覧 を通し、オフライン起動も見る。**ユーザーの `data/` は Gemini のモデル名と RPM/TPM/RPD が未入力なので、先に設定タブで入れてもらう**（`gemini-2.5-flash` で動作確認済み）。そのあと P11（財務数値の取り込み。SPEC §2.9）。**P11-4 まで進んだら `report.render_report(..., financials=...)` に値を渡すだけで財務の図が出る** |
| **リポジトリ状態** | **`main` に P12（レポートの視覚強化とアプリ内ビューア）までマージして push 済み**（`dd5b5d0`）。P11（財務数値）は**設計だけが入っていて実装は未着手**。次の作業は `main` から新しいブランチを切って始める。コミットのメールアドレスはリポジトリ設定で GitHub の noreply アドレスにしてある（個人アドレスだと GitHub が push を拒否する） |
