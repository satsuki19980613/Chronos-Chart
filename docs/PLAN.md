...
|---|---|
| **現在のフェーズ** | P7（仕上げ） |
| **次にやること** | **P11-1〜P11-7（財務数値の取り込み → 指標 → プロンプト → レポートへの反映）。2026-09-21 にユーザーが「次のセッションでは財務分析の取得からレポート反映までを行う」と指示した**ので、次はここから始める（`feature/p12-lwc-report` のマージと push が先。ユーザーの指示待ち）。**P7-2（通しの動作確認）は後回し**にする（Gemini のモデル名と RPM/TPM/RPD をユーザーが設定タブへ入力しないと進められないため）。P7-2 は実機（`start.bat`）で 起動→登録→需給取得→開示取得→チャート→AI 分析→レポート閲覧 を通し、オフライン起動も見る。**ユーザーの `data/` は Gemini のモデル名と RPM/TPM/RPD が未入力なので、先に設定タブで入れてもらう**（`gemini-2.5-flash` で動作確認済み）。そのあと P11（財務数値の取り込み。SPEC §2.9）。**P11-4 まで進んだら `report.render_report(..., financials=...)` に値を渡すだけで財務の図が出る** |
| **リポジトリ状態** | **`main` に P12-1〜P12-4 までマージして push 済み**（`3df22a0`）。**作業ブランチ `feature/p12-lwc-report` に P12-5・P12-6（株価チャートを Lightweight Charts に差し替え）がコミット済みで、マージと push はユーザーの指示待ち。** P11（財務数値）は**設計だけが入っていて実装は未着手**。コミットのメールアドレスはリポジトリ設定で GitHub の noreply アドレスにしてある（個人アドレスだと GitHub が push を拒否する） |
| **外部アクセスの消費** | 2026-09-20 に採取済み: karauri.net `/6920/` を1回、`taisyaku.jp` の `zandaka.csv` `meigara.csv` を各1回、**EDINET の利用規約ページ（閲覧）と `Edinetcode.zip` を各1回**。実物は `tests/fixtures/real/`（Git 対象外）。**以後これらへはアクセスしない**。2026-09-20 に API キー取得後、**EDINET API v2 の `documents.json` を31回**（疎通確認1 + 実測30日分）。これは API の正規の使い方なので回数制限は無いが、1秒間隔は守ること。karauri の User-Agent の連絡先はユーザー指定でリポジトリ URL `https://github.com/satsuki19980613/Chronos-Chart` 。2026-09-20 に P8 の検証で、**EDINET コードリストを1回**（アプリの正規の取得経路）、**`documents.json` を366日分**（API の正規の使い方・1秒間隔）、**karauri.net の `/9984/` を1回**（10秒間隔・UA に連絡先）取得した。ユーザーの `data/` の `scrape_contact` にリポジトリ URL を設定済み（`auto_update_short` はオフのまま）。2026-09-21 に P9 の検証で**書類取得 API（`documents/<docID>?type=1`）を7回**（構造確認2・実測1・画面確認4。いずれも API の正規の使い方・1秒間隔）。2026-09-21 に P6 で **Gemini API を16回**（`models.list` 1・`count_tokens` 7・`generate_content` 8。構造の採取、`ping()` の疎通、実データのプロンプトのトークン見積り、画面からの分析1回と接続テスト1回。ユーザーの無料枠を使うので最小限に留め、**429 は故意に起こしていない**）。2026-09-21 に P11 の設計のため **書類取得 API の `type=5` を2回**（有報の CSV。IFRS の 9984 と日本基準の1社。構造確認のみで保存しない）。**サブエージェントの調査では EDINET・karauri・taisyaku へのアクセスを禁じた**（回数はメインが管理する） |
...
| ID | 状態 | タスク | 完了条件 | 依存 | 対象 |
|---|---|---|---|---|---|
| P11-1 | `TODO` | `type=5` CSV の取得とパース | SPEC §2.9.1〜2.9.4。`csvFlag` で判定し追加リクエストを増やさない。**UTF-16LE のタブ区切り**、`XBRL_TO_CSV/jpcrp*-asr-*`（半期報は `ssr`）を選ぶ。**接尾辞なしのコンテキストIDだけを採用**し `_NonConsolidatedMember` は単体、その他の `Member` は捨てる。標準要素32種の対応表。ZIP 20MB 上限は P9-1 と共通。合成フィクスチャで `test_edinet.py` | P9-1 | `sources/edinet.py` `tests/fixtures/` |
| P11-2 | `TODO` | `financials` テーブルと保存 | SPEC §2.9.5・§3。移行 **v6**。`(symbol, period_end, item, basis)` 主キー。**同じ会計期間は提出日が新しい書類で上書き、訂正に無い項目は残す**。連結が1件も無ければ単体にフォールバックし `basis` に記録。銘柄削除で CASCADE | P11-1 | `database.py` `financials.py` |
| P11-3 | `TODO` | 取得ジョブと導線 | `financials` ジョブ（進捗・中断）。未取得の有報・半期報だけを取る。銘柄登録直後と、開示取得のあとに案内する。自動更新に**入れない**（年2回しか増えないため） | P11-2 P1-6 | `financials.py` `api.py` `app.js` |
| P11-4 | `TODO` | 指標の算出 | SPEC §2.9.6。開示済みの ROE・自己資本比率・PER・配当性向は**そのまま採用**。成長率・CAGR・営業CFマージン・FCF・アクルーアル・自己ヒストリカルの位置・トレンド3値を算出。**会社予想に関する指標は作らない**。`test_financials.py` | P11-2 | `financials.py` |
...
### 財務数値（P11。実データで確認済み。再採取しないこと）

`documents/<docID>?type=5` の実物を2件（IFRS・日本基準）見て分かったこと。**この4点を忘れると必ず躓く。**

1. **文字コードは UTF-16LE（BOM あり）でタブ区切り。** 拡張子は `.csv` だが実体は TSV。
   `raw.decode("utf-16")` してから `csv.DictReader(..., delimiter="	")` で読める
2. ZIP の中は `XBRL_TO_CSV/`。**本体は `jpcrp030000-asr-*`**（半期報は `jpcrp050000-ssr-*` のはず。未確認＝§5 #10）。
   `jpaud-*` は監査報告書なので使わない
3. **`連結・個別` 列で連結と単体を区別できない**（どちらも `その他` になる）。
   判定は**コンテキストID**: 接尾辞なし＝連結、`_NonConsolidatedMember`＝単体、その他の `...Member`＝セグメント等（捨てる）
4. `csvFlag` は**既に日次キャッシュの中にある**ので、CSV の有無を調べるために書類取得 API を叩く必要はない

列は `要素ID / 項目名 / コンテキストID / 相対年度 / 連結・個別 / 期間・時点 / ユニットID / 単位 / 値` の9つ。
「主要な経営指標等の推移」は `jpcrp_cor:...SummaryOfBusinessResults` で、実測32種・最大5期分（`Prior4Year` まで）。
**ROE・自己資本比率・PER・配当性向は発行体が算出した値がそのまま入っている**ので再計算しない。
IFRS 採用企業は連結が `...IFRSSummaryOfBusinessResults`、単体が接尾辞なしの要素名になる（単体は日本基準のため）。

規模の実測: 有報1件で ZIP 248KB・CSV 2,237行（IFRS）／ ZIP 127KB・CSV 1,842行（日本基準）。
...
| 10 | `OPEN` | 「主要な経営指標等の推移」が常に標準要素だけで揃うか。**半期報告書の `type=5` の構造は未確認**（有報2件でしか見ていない） | P11-1 で半期報告書を1回採取して確認する。揃わない企業があっても、取れた項目だけで指標を出す設計にしておく | P11-1 |
| 11 | `OPEN` | 連結財務諸表を作っていない会社（単体のみ）で、コンテキストが `_NonConsolidatedMember` だけになるのか、それとも接尾辞なしで入るのか | P11-1 の実装時に該当する会社の書類を1回だけ確認する。どちらでも動くよう「接尾辞なしを優先し、無ければ単体」のフォールバックにしておく | P11-1 |
