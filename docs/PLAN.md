...
|---|---|
| **現在のフェーズ** | P7（仕上げ） |
| **次にやること** | **P6 は完了。残るは P7-2（通しの動作確認）だけ。** 起動（自動更新が走る）→ 登録 → 需給取得 → 開示取得 → チャート表示 → AI 分析 を**実機（`start.bat` の pywebview ウィンドウ）**で通し、オフラインでの起動も確認する。ユーザーの `data/` では **Gemini のモデル名と RPM/TPM/RPD が未入力**なので、先に設定タブで入れてもらう必要がある（AI Studio のレート制限ページの値。モデルは `gemini-2.5-flash` で動作確認済み） |
