...
| **次にやること** | **P5-1（イベント変換）は完了。次は P5-2（チャートマーカー）・P5-3（連動用 API）。** SPEC §2.5.3。売買シグナルと同一配列にマージして `createSeriesMarkers` を1回だけ呼ぶ。OVERLAYS に「開示」チップ（既定 ON）。`render()` の戻り値に `onVisibleRangeChange` / `scrollToDate` / `onMarkerClick` |
