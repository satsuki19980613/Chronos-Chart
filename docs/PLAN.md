...
| **次にやること** | **P8（実機テストで見つかった不備）を先に片付ける。** ユーザーの実機テストで (1) `edinet_codes` が常に空（コードリストの取得がどこからも呼ばれていない）→ 大量保有報告書が永久に紐づかない、(2) 新規登録した銘柄の過去開示が取り込まれない（自動更新は直近30日・登録時はキャッシュ再走査のみ）、(3) 空売り残高は `scrape_contact` 未設定で無効なのに理由がツールチップにしか出ない、が判明した。P8-1〜P8-4 を終えてから P6-1（Gemini クライアント）へ |
