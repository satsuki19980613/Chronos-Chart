...
|---|---|
| **現在のフェーズ** | P7（仕上げ） |
| **次にやること** | **P7-2（通しの動作確認）→ そのあと P11-1。** P7-2 は実機（`start.bat`）で 起動→登録→需給取得→開示取得→チャート→AI 分析 を通し、オフライン起動も見る。**ユーザーの `data/` は Gemini のモデル名と RPM/TPM/RPD が未入力なので、先に設定タブで入れてもらう**（`gemini-2.5-flash` で動作確認済み）。そのあと P11（財務数値の取り込みと指標。SPEC §2.9）に入る |
