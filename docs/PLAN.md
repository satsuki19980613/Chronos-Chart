...
|---|---|
| **現在のフェーズ** | P7（仕上げ） |
| **次にやること** | **P11-5（プロンプトへの統合とテクニカルの圧縮）から。** P11-1〜P11-4（取得・保存・ジョブ・指標）は完了し、実機の「財務数値を取得」ボタンから EDINET の有報・半期報を取り込めるところまで通っている。P11-5 では、財務を1指標1行で足しつつ**テクニカルを圧縮して総トークンを今より減らす**ことを実測で確かめる（SPEC §2.9.7・§2.9.8）。そのあと P11-6（`fundamental` と `data_scope_note`・レポートの財務表・数値の実在検証）→ P11-7（実データ4パターンでの通し確認）。**P7-2（通しの動作確認）は後回し**（Gemini のモデル名と RPM/TPM/RPD をユーザーが設定タブへ入力しないと進められない。`gemini-2.5-flash` で動作確認済み） |
...
| P11-5 | `TODO` | プロンプトへの統合とテクニカルの圧縮 | SPEC §2.9.7・§2.9.8。財務は1指標1行（値・前期比・トレンド）。会計基準と連結/単体と期ずれを明記。**同時にテクニカルを圧縮**（直近20日は生、それ以前はシグナル発生日＋要約統計）し、**財務を足しても総トークンが今より減る**ことを実測で確認する。番兵値テストは維持 | P11-4 P6-3 | `ai/prompt.py` |
| P11-6 | `TODO` | 出力スキーマとレポート、数値の実在検証 | SPEC §2.9.9。`fundamental` を `technical` と `disclosure` の間に、`data_scope_note` を末尾に追加。レポートに財務指標の表を出す。**AI が根拠に書いた数値がプロンプトに実在するかを照合**し、結果をレポートに出す | P11-5 P6-5 | `ai/schema.py` `ai/report.py` `templates/` |
