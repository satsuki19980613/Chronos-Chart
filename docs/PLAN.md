...
|---|---|
| **現在のフェーズ** | P7（仕上げ） |
| **次にやること** | **P11 は完了（P11-1〜P11-7）。** 次は **P7-2（実機での通しの動作確認）** か **P10（画面の細かい改善）**。P7-2 は `start.bat` で 起動→登録→需給取得→開示取得→財務数値取得→チャート→AI 分析→レポート閲覧 を通し、オフライン起動も見る。**先にユーザーが設定タブで Gemini のモデル名と RPM/TPM/RPD を入れる必要がある**（`gemini-2.5-flash` で動作確認済み）。AI 分析を1回実行して、`fundamental` と `data_scope_note` と数値の実在検証が実レスポンスでも期待どおり出るかを見るのが残りの確認 |
| **リポジトリ状態** | `main` に P12 までマージして push 済み。**P11 は `feature/p11-financials` ブランチに3コミットで全部入っている（未マージ・未 push）**。コミットのメールアドレスはリポジトリ設定で GitHub の noreply アドレスにしてある |
| **外部アクセスの消費** | 2026-09-20 に採取済み: karauri.net `/6920/` を1回、`taisyaku.jp` の `zandaka.csv` `meigara.csv` を各1回、**EDINET の利用規約ページ（閲覧）と `Edinetcode.zip` を各1回**。実物は `tests/fixtures/real/`（Git 対象外）。**以後これらへはアクセスしない**。2026-09-20 に API キー取得後、**EDINET API v2 の `documents.json` を31回**（疎通確認1 + 実測30日分）。これは API の正規の使い方なので回数制限は無いが、1秒間隔は守ること。karauri の User-Agent の連絡先はユーザー指定でリポジトリ URL `https://github.com/satsuki19980613/Chronos-Chart` 。2026-09-20 に P8 の検証で、**EDINET コードリストを1回**（アプリの正規の取得経路）、**`documents.json` を366日分**（API の正規の使い方・1秒間隔）、**karauri.net の `/9984/` を1回**（10秒間隔・UA に連絡先）取得した。ユーザーの `data/` の `scrape_contact` にリポジトリ URL を設定済み（`auto_update_short` はオフのまま）。2026-09-21 に P9 の検証で**書類取得 API（`documents/<docID>?type=1`）を7回**（構造確認2・実測1・画面確認4。いずれも API の正規の使い方・1秒間隔）。2026-09-21 に P6 で **Gemini API を16回**（`models.list` 1・`count_tokens` 7・`generate_content` 8。構造の採取、`ping()` の疎通、実データのプロンプトのトークン見積り、画面からの分析1回と接続テスト1回。ユーザーの無料枠を使うので最小限に留め、**429 は故意に起こしていない**）。2026-09-21 に P11 の設計のため **書類取得 API の `type=5` を2回**（有報の CSV。IFRS の 9984 と日本基準の1社。構造確認のみで保存しない）。2026-09-21 に P11-1 の実装前に **`type=5` を4回**（IFRS 有報・日本基準 有報・**半期報**・**連結決算のない会社の有報**。いずれも API の正規の使い方・1.5秒間隔）。**この4件は `tests/fixtures/real/` に保存した**ので、以後は再取得せずそこを読むこと。さらに P11-3 の実機確認（開発サーバーの「財務数値を取得」ボタン）で **`type=5` を2回**（9984 の有報と半期報。アプリの正規の取得経路）。P11-5 のトークン実測で **Gemini の `count_tokens` を2回**（60日・120日のプロンプト。`generate_content` は呼んでいない）。**サブエージェントの調査では EDINET・karauri・taisyaku へのアクセスを禁じた**（回数はメインが管理する） |
