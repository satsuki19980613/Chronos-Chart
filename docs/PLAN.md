...
| **次にやること** | P4-5（自動更新への開示の組み込み）。SPEC §2.8.2 の順3。キー未設定は黙ってスキップ、未取得が30日を超える場合は `max_days=30` で直近30日分だけ。`test_autoupdate.py` に追加。これで P4 が完了する |
