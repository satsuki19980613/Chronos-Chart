...
| P10 | 画面の細かい改善（ユーザー要望） | WIP |
| P11 | 財務数値の取り込みと指標（ユーザー要望） | DONE |
| P12 | レポートの視覚強化とアプリ内ビューア（ユーザー要望） | DONE |
...
| P6-6 | `DONE` | レポートタブUI | `ai_analyze` ジョブ、実行前の確認ダイアログ（見積り・残量・送信データ種別）、進捗、レポート一覧と「開く」、設定タブの Gemini 接続テストと打ち切り解除ボタン | P6-5 P1-6 P1-8 | `api.py` `app.js` `index.html` |
...
| P7-2 | `WIP` | 通しの動作確認 | 起動（自動更新が走る）→ 登録 → 需給取得 → 開示取得 → チャート表示 → AI分析 を実機で通す。オフラインでの起動も確認。`python -m pytest` が全通過 | P6-6 P5-5 P3-4 P7-1 | — |
...
### P9 — 開示のモーダルと本文表示（2026-09-21 ユーザー要望）

| ID | 状態 | タスク | 完了条件 | 依存 | 対象 |
|---|---|---|---|---|---|
| P9-1 | `DONE` | 書類本文の取得とテキスト化 | SPEC §2.4.8。`documents/<docID>?type=1` の ZIP をメモリで取得し `XBRL/PublicDoc/*.htm` をファイル名昇順でテキスト化。ZIP 20MB・テキスト20万文字の上限、読み取り60秒、失敗はリンク誘導（401/403/429 はエラー）。プロセス内キャッシュ16件。`test_edinet.py` | P4-1 | `sources/edinet.py` |
| P9-2 | `DONE` | `get_disclosure_text` API | SPEC §4.2。キー未設定は `UserFacingError`。戻り値は `{doc_id, available, reason, title, text, truncated, url}`。`test_api_disclosures.py` | P9-1 | `api.py` |
| P9-3 | `DONE` | 開示モーダル | マーカークリックとイベント欄の「開く」でモーダルを開く。その日の全件をメタ情報付きで並べ、本文をテキスト表示（1件のときは自動読み込み）。出せないものは「EDINET で開く」に誘導。出典と加工主体を明記。Esc・外側クリック・閉じるで閉じる | P9-2 P5-5 | `app.js` `index.html` `style.css` |
| P9-4 | `DONE` | 実データでの確認 | 臨報・大量保有・有報の3種でモーダルの本文表示とリンク誘導を確認 | P9-3 | — |

### P10 — 画面の細かい改善（ユーザー要望）

| ID | 状態 | タスク | 完了条件 | 依存 | 対象 |
|---|---|---|---|---|---|
| P10-1 | `DONE` | 日次データをチャートの下へ | 右カラム（テクニカル判定・シグナル・開示イベント）のほうが縦に長く、チャート下に大きな余白ができていた。日次データのパネルを `.dash-grid` の左カラム（`.dash-main`）へ移して余白を埋める | P5-4 | `index.html` `style.css` |

---

### P11 — 財務数値の取り込みと指標（2026-09-21 ユーザー要望）

SPEC §2.9。**生の財務諸表は AI に送らず、アプリ側で指標に落として1指標1行で渡す。**
調査の生メモは `docs/research/`（仕様の正は SPEC）。

| ID | 状態 | タスク | 完了条件 | 依存 | 対象 |
|---|---|---|---|---|---|
| P11-1 | `DONE` | `type=5` CSV の取得とパース | SPEC §2.9.1〜2.9.4。`csvFlag` で判定し追加リクエストを増やさない。**UTF-16LE のタブ区切り**、`XBRL_TO_CSV/jpcrp<6桁>-asr-*`（半期報は **`jpcrp040300-ssr-*`**）を選ぶ。**接尾辞なしのコンテキストIDだけを採用**し `_NonConsolidatedMember` は単体、その他の `Member` は捨てる。標準要素の対応表（**IFRS の `EquityToAssetRatioIFRS…` は BPS**・比率は100倍・PER は倍率）。ZIP 20MB 上限は P9-1 と共通。合成フィクスチャで `test_edinet.py` | P9-1 | `sources/edinet.py` `tests/fixtures/` |
| P11-2 | `DONE` | `financials` テーブルと保存 | SPEC §2.9.5・§3。移行 **v6**（`financials` と `financial_docs`）。`period_type` で通期と中間期を分ける。`(symbol, period_end, item, basis)` 主キー。**同じ会計期間は提出日が新しい書類で上書き、訂正に無い項目は残す**。連結が1件も無ければ単体にフォールバックし `basis` に記録。銘柄削除で CASCADE | P11-1 | `database.py` `financials.py` |
| P11-3 | `DONE` | 取得ジョブと導線 | `financials` ジョブ（進捗・中断）。未取得の有報・半期報だけを取る。銘柄登録直後と、開示取得のあとに案内する。自動更新に**入れない**（年2回しか増えないため） | P11-2 P1-6 | `financials.py` `api.py` `app.js` |
| P11-4 | `DONE` | 指標の算出 | SPEC §2.9.6。開示済みの ROE・自己資本比率・PER・配当性向は**そのまま採用**。成長率・CAGR・営業CFマージン・FCF・アクルーアル・自己ヒストリカルの位置・トレンド3値を算出。**会社予想に関する指標は作らない**。`test_financial_metrics.py` | P11-2 | `financial_metrics.py` |
| P11-5 | `DONE` | プロンプトへの統合とテクニカルの圧縮 | SPEC §2.9.7・§2.9.8。財務は1指標1行（値・前期比・トレンド）。会計基準と連結/単体と期ずれを明記。**同時にテクニカルを圧縮**（直近20日は生、それ以前はシグナル発生日＋要約統計）し、**財務を足しても総トークンが今より減る**ことを実測で確認する。番兵値テストは維持 | P11-4 P6-3 | `ai/prompt.py` |
| P11-6 | `DONE` | 出力スキーマとレポート、数値の実在検証 | SPEC §2.9.9。`fundamental` を `technical` と `disclosure` の間に、`data_scope_note` を末尾に追加。レポートに財務指標の表を出す。**AI が根拠に書いた数値がプロンプトに実在するかを照合**し、結果をレポートに出す | P11-5 P6-5 | `ai/schema.py` `ai/report.py` `templates/` |
| P11-7 | `DONE` | 実データでの確認 | 日本基準・IFRS・単体のみ・財務なしの4パターンで、取り込み → 指標 → プロンプト → レポートを通す。トークン数の前後比較を記録する | P11-6 | — |

---

### P12 — レポートの視覚強化とアプリ内ビューア（2026-09-21 ユーザー要望）

SPEC §2.7.6。ユーザーの指摘「グラフや図や表がないのは非常に見づらい」「レポートタブから HTML を見れるようにしてほしい。
フォルダに飛ぶのは UX が悪すぎる」。調査メモは `docs/research/report-visual-design.md` と `svg-and-viewer.md`。

| ID | 状態 | タスク | 完了条件 | 依存 | 対象 |
|---|---|---|---|---|---|
| P12-1 | `DONE` | レポート用のインライン SVG | 依存を増やさず標準ライブラリだけで生成。スパークライン・折れ線（マーカー付き）・棒＋折れ線・群棒・バレット・プレースホルダ。Okabe–Ito、`currentColor` と `--chart-N`、`role="img"`＋`<title>`/`<desc>`、**棒の軸は 0 始まり**、`<script>` を出さない。`test_charts.py` | P6-5 | `ai/charts.py` |
| P12-2 | `DONE` | レポートの構成改訂 | SPEC §2.7.6。株価チャート・財務ハイライト（KPI カード＋4図）・テクニカルのバッジ・指標表のカテゴリ分けと `<details>` 折りたたみ・印刷 CSS・ダークモード。**財務は `financials=None` でプレースホルダ**（P11 が値を渡せば図が出る）。`test_report.py` | P12-1 | `ai/report.py` `templates/report.html.j2` |
| P12-3 | `DONE` | アプリ内ビューア | `get_report_html` を追加し、レポートタブを一覧＋プレビューの2ペインに。**`sandbox=""` の iframe に `srcdoc`**。一覧はカード形式（キーボード操作可）。分析完了後はできたてを自動表示。「ブラウザで開く」は印刷用に残し、「保存フォルダを開く」は補助に降格 | P6-6 | `api.py` `index.html` `app.js` `style.css` |
| P12-4 | `DONE` | 実データでの確認 | 実データ＋5期分の財務でレポートを生成し、ダーク／ライト・アプリ内ビューア・一覧の収まりを確認 | P12-2 P12-3 | — |
| P12-5 | `DONE` | 株価チャートを Lightweight Charts に差し替え | SPEC §2.7.6。ユーザーの指摘「SVG の株価チャートは視認性が悪い。ダッシュボードと同じトレーディングビューの UI にしてほしい」「プレビューの白い枠線は不要」。同梱ライブラリをレポートに**インライン埋め込み**し、ローソク足＋SMA＋出来高＋マーカーを描く。データは `<script type="application/json">`（`<`・`>`・`&` をエスケープ）で渡し、**AI の出力はスクリプト文脈に入れない**。iframe を `sandbox="allow-scripts"` に（`allow-same-origin` は付けない）。**JS が動かないときは既存の SVG 折れ線にフォールバック**。プレビューの白い枠・余白・背景を撤去。`test_price_chart.py` | P12-3 | `ai/price_chart.py` `templates/report_chart.js` `ai/report.py` `templates/report.html.j2` `index.html` `style.css` |
| P12-6 | `DONE` | 差し替え後の実データ確認 | 実データ（9984.T・直近60日）でレポートを生成し、**アプリ内プレビュー（`sandbox="allow-scripts"` の iframe）でローソク足・移動平均・出来高・マーカーが描画されること**、ブラウザでダーク/ライト両方を確認。白い枠が消えたことも確認。生成 HTML は **272KB**（うちライブラリ 198KB） | P12-5 | — |
...
| 17 | 2026-09-21 | P12-1〜P12-4 | **レポートに図を入れ、アプリ内で読めるようにした**（ユーザー要望）。調査2本（`docs/research/`）→ Sonnet 3本で実装。図は**依存を増やさないインライン SVG**（レポートは外部参照ゼロの単一 HTML なので JS ライブラリを使えない）。**`data/reports/` はどちらのサーバーからも配信されない**と分かったので、HTML 文字列を API で渡して `sandbox=""` の iframe に `srcdoc` で入れる方式にした。メインの実機レビューで、(1) 一覧の表が左カラムからはみ出して操作ボタンが隠れていた（カード形式に変更）、(2) 比率の増減を「%の%」で出していた（ポイント差に）、(3) 増減の色（増加＝赤）が誤読を招く（色を外し ▲▼ のみ）、(4) 空系列が凡例に残る、(5) SMA25/75 の色が近い、(6) 空の KPI に理由が無い、を修正。958 passed / 15 skipped |
| 18 | 2026-09-21 | P12-5, P12-6 | **レポートの株価チャートを Lightweight Charts のローソク足に差し替えた**（ユーザーの指摘「SVG の株価チャートは視認性が悪い。ダッシュボードのトレーディングビューの UI を使ってほしい」「プレビューの白い枠線は不要」）。同梱ライブラリ 198KB を**レポートにインライン埋め込み**し、iframe を `sandbox="allow-scripts"` に変更（`allow-same-origin` は付けない）。データは `<script type="application/json">` 経由で、**AI の出力はスクリプト文脈に入れない**。Sonnet 2本（チャートのモジュールと初期化スクリプト／レポートへの組み込み）で実装。メインのレビューで、(1) `beforeprint` では印刷用 CSS の値をまだ読めない（印刷中は CSS 変数を読まず原色＋白背景に固定）、(2) 下段ペインの高さ配分（`setStretchFactor`）がダッシュボードと違う、(3) 初期化の途中で失敗すると SVG フォールバックごと消える、(4) 出来高の色がローソク足と揃わない、を修正。実データの確認で **ローソク足の陽線（Okabe–Ito の vermilion）が SMA5（orange）と見分けづらい**ことが分かり、ローソク足だけ専用の `--candle-up` / `--candle-down`（ライト・印刷は濃い赤と青、ダークはダッシュボードと同じ）に変えた。993 passed / 15 skipped |
| 16 | 2026-09-21 | （P11 の設計） | ユーザーの要望で **XBRL 財務数値の DB 化**に着手することになり、設計を固めた。Sonnet のサブエージェント4本で調査し（`docs/research/`）、メインが実データ2件で裏取りした。要点: (1) `type=5` の CSV は **UTF-16LE のタブ区切り**、(2) 「主要な経営指標等の推移」が**標準要素32種・5期分**で取れ、**ROE・自己資本比率・PER は発行体の算出値がそのまま載っている**、(3) **連結/単体の判別は `連結・個別` 列ではできずコンテキストIDで行う**、(4) LLM は数値計算が苦手で長い入力ほど劣化するため**計算はアプリ側に寄せ、テクニカルを圧縮して総トークンをむしろ減らす**。「LLM は財務諸表分析でアナリストを超える」の根拠論文が**撤回済み**と判明したのも大きい。SPEC §2.9 と P11（7タスク）を起こした |
| 15 | 2026-09-21 | P6-1〜P6-6 | **P6（AI 分析レポート）完了。** 実レスポンスを採取して §9-4 を解消（SPEC §2.7.2a）。Sonnet のサブエージェント6本（クライアント／クォータ／スキーマとプロンプト／検証と再依頼／レポート生成／画面）で実装し、メインが API・ジョブ登録・差分レビュー・実機確認を担当した。**実行の流れは `app/ai/analyze.py` に分離**し、`client.py` を「1呼び出し＝1送信」に保った（SPEC §2.7.5 に5番として追記）。メインのレビューで、(1) クォータのトークン補正が RPM を二重計上していたバグ、(2) `count_tokens` が SDK の例外を素通りさせていた点、(3) プロンプトに指標の定義が無く列名の略号だけだった点、(4) レポートの指標値の桁と role の生表示、(5) レポートタブの既定銘柄を修正。実 Gemini で通しの確認まで実施。874 passed / 15 skipped |
| 14 | 2026-09-21 | P10-1 | 日次データのパネルをダッシュボード左カラム（チャートの下）へ移動。右カラムのほうが長く、チャート下が大きく空いていたため（ユーザーがスクリーンショットで指摘）。`.dash-main` を追加し、`min-width: 0` で表の横スクロールが列幅を押し広げないようにした |
| 13 | 2026-09-21 | P9-1〜P9-4 | **開示モーダルと本文のテキスト表示**（ユーザー要望）。規約上 API 経由の取得は許されている（§2.4.7）ので書類取得 API（`type=1`）の ZIP をメモリで展開し、`PublicDoc` の inline XBRL からテキストを抽出して表示する（**ディスクには保存しない**。プロセス内 LRU 16件）。上限は ZIP 20MB・テキスト20万字、読み取り60秒。テキストにできない・大きすぎる・失敗は「EDINET で開く」に誘導し、401/403/429 だけエラー扱い。`HttpClient.get` に `read_timeout` / `max_bytes` を追加（既定の挙動は不変）。マーカークリックでその日の全件、イベント欄の「開く」で1件のモーダルを開く。メインが実データで検証し、`<title>` のファイル名混入と `PublicDoc/`（`XBRL/` 無し）の取りこぼしを修正。704 passed / 15 skipped |
...
### AI 分析（P6-1〜P6-3 で実装済み。P6-4 以降はこれを呼ぶ）

- `app/ai/client.py` — **1回の呼び出しにつき送信は1回だけ**。再送・再依頼のループは呼び出し側の責務。
  `GeminiClient.from_settings(settings)` / `count_tokens(prompt)` / `generate(prompt, schema)` / `ping()`。
  戻り値 `Reply(text, finish_reason, usage, truncated)`、`Usage(prompt_tokens, output_tokens, thoughts_tokens, total_tokens)`。
  **`output_tokens` は candidates + thoughts**（枠を消費するのはこちら）。429 は `QuotaExceeded`（`.hit` に `QuotaHit`）。
  SDK の実挙動は SPEC §2.7.2a
- `app/ai/quota.py` — `Quota(db, settings)` が窓口。**送信直前に `start_request(need_tokens)`**（check → wait → requests 加算）、
  応答後に `finish_request(in, out)`。再依頼のたびに `start_request` を通すこと（まとめて確保しない）。
  429 を受けたら `apply_quota_hit(db, model, scope)`（`per_minute` 以外は打ち切り）。
  `snapshot()` が画面用の残量。RPM/TPM の窓はプロセス内メモリ、RPD は `ai_usage`
- `app/ai/prompt.py` — `PromptSource(db).load(symbol, days)` → 凍結 `PromptInput` → `build_prompt` / `build_retry_prompt`。
  **需給テーブルを読む経路が無い**ことがこのモジュールの担保。`days` は 20/60/120 のみ。
  **P11-5 で圧縮した。** 直近20日は生 CSV、それ以前は要約（四本値・出来高・シグナル・指標ごとの方向）。
  `PromptInput` に `financials`（`financial_metrics.compute_metrics` の戻り値）と `compressed` を持つ。
  実測（9984.T・実データ）: **60日 28,420→15,355 トークン / 120日 約53,000→16,305 トークン**。
  20日は圧縮区間が無いので財務のぶんだけ増える（想定どおり）
...
- `app/ai/verify.py` — `verify_numbers(prompt_text, report)`。**実際に送ったプロンプト**と突き合わせる。
  日付は分解しない・丸め違いは許す・単位の言い換えは追わない・1〜10 の単独整数は除く（SPEC §2.9.9）
- `app/financials.py` — 保存（`save_financials`・訂正は提出日時が新しいほうだけ上書き）／`load_series`（通期と中間期を分けて返す）／`financials_job`／`estimate_financials`
- `app/financial_metrics.py` — `compute_metrics(series)`。**DB に触らない純関数**。比率の前期比はポイント差、比率と倍率に CAGR は出さない
...
### 財務数値（P11。実データ4件で確認済み。**再採取しないこと**）

`documents/<docID>?type=5` の実物を **4件**（IFRS 有報・日本基準 有報・**半期報告書**・**連結決算のない会社の有報**）見て
分かったこと。**実物は `tests/fixtures/real/` に置いてある**（Git 対象外）ので、確かめたくなったら再取得せずそこを読む。

| ファイル | 中身 |
|---|---|
| `S100YGH5.zip` | ソフトバンクグループ 有報・IFRS・連結あり |
| `S100Y62Z.zip` | タキヒヨー 有報・日本基準・連結あり |
| `S100Z1Z3.zip` | モロゾフ **半期報告書**・日本基準 |
| `S100Z2OT.zip` | ユーザーローカル 有報・**連結決算なし（単体のみ）** |

**これを忘れると必ず躓く点**（詳細と根拠は SPEC §2.9.2〜§2.9.4a）:

1. **文字コードは UTF-16LE（BOM あり）でタブ区切り。** 拡張子は `.csv` だが実体は TSV
2. 本体は **`jpcrp<6桁>-asr-*`（有報）／ `jpcrp<6桁>-ssr-*`（半期報）**。
   半期報の実物は `jpcrp040300-ssr-001` だった（当初 SPEC に書いた `jpcrp050000-ssr` は**誤り**）。`jpaud-*` は監査報告書
3. **`連結・個別` 列で連結と単体を区別できない**（どちらも `その他`）。判定は**コンテキストID**:
   接尾辞なし＝連結、`_NonConsolidatedMember`＝単体、その他の `...Member`＝セグメント等（捨てる）
4. **`EquityToAssetRatioIFRSSummaryOfBusinessResults` は自己資本比率ではなく BPS。**
   項目名は「１株当たり親会社所有者帰属持分（IFRS）」・単位 `JPYPerShares`。
   IFRS の本当の自己資本比率は `RatioOfOwnersEquityToGrossAssetsIFRS…`
5. **比率は `pure` 単位の小数**（自己資本比率 `0.638` = 63.8%）なので100倍する。
   **PER だけは倍率**（`11.7` = 11.7倍）なので100倍しない
6. **`dps` / `payout_ratio` / `shares_outstanding` / `capital_stock` は連結側に存在しない。**
   「提出会社の状況」の表にしか無いので、連結を採用した書類でも `_NonConsolidatedMember` から補う
7. **半期報告書は中間期（6か月）と前事業年度（12か月）の数値が同居する。**
   1本の系列に混ぜると前期比が「半年 ÷ 1年」になるので、`period_type`（`FY`/`HY`）で必ず分ける
8. **欠損値は全角ハイフン `－`（U+FF0D）**。空文字ではない
9. 会計基準・期間末日・連結決算の有無は **`jpdei_cor:` の DEI 行**から取る。CSV に日付の列は無い
10. `csvFlag` は**既に日次キャッシュの中にある**ので、CSV の有無を調べるために書類取得 API を叩く必要はない

規模の実測: ZIP 248KB/2,236行（IFRS 有報）・127KB/1,842行（日本基準 有報）・67KB/815行（単体のみ）・22KB/479行（半期報）。
...
| 3 | `CLOSED` | `usage_metadata` のフィールド名、429 エラー詳細の実構造、thinking 予算の指定方法 | P6-1 で実レスポンスを採取。SPEC §2.7.2a に記載（`count_tokens().total_tokens`／`prompt_token_count`・`candidates_token_count`・`thoughts_token_count`・`total_token_count`／`finish_reason` は列挙型で `MAX_TOKENS` でも `text` は入る／`ThinkingConfig(thinking_budget=N)` は 2.5 系・3 系のどちらも可）。**429 だけは未採取**（無料枠を故意に使い切らないため）で、`ClientError.details` からの読み取りを欠損耐性つきにして吸収した | 解消（2026-09-21） |
...
| 10 | `CLOSED` | 「主要な経営指標等の推移」が常に標準要素だけで揃うか。半期報告書の `type=5` の構造 | P11-1 の前にメインが半期報告書を1回採取して確認。本体は **`jpcrp040300-ssr-*`**（SPEC の `jpcrp050000-ssr` は誤りだったので訂正）。コンテキストは `Interim*` / `Prior1Interim*` / `Prior1Year*` の3種で、**ROE・PER・配当性向・発行済株式数・資本金は載らない**。取れた項目だけで指標を出す設計にしてあるので問題ない（SPEC §2.9.2・§2.9.4） | 解消（2026-09-21） |
| 11 | `CLOSED` | 連結財務諸表を作っていない会社（単体のみ）で、コンテキストが `_NonConsolidatedMember` だけになるのか | メインが該当する会社（`WhetherConsolidatedFinancialStatementsArePreparedDEI` が `false`）の有報を1回採取して確認。**`_NonConsolidatedMember` のコンテキストしか存在しなかった**ので、「接尾辞なしを優先し、無ければ単体」のフォールバックが必須。あわせて、連結側に `dps`・`payout_ratio`・`shares_outstanding`・`capital_stock` が**そもそも無い**ことも分かった（SPEC §2.9.4） | 解消（2026-09-21） |
...
| 2026-09-21 | レポートの図は**インライン SVG を自前で生成**する（matplotlib・pygal 等を入れない） | レポートは外部参照ゼロの単一 HTML で、JS のチャートライブラリも CDN も使えない。実測でスパークライン 0.5KB・60日の折れ線 4.3KB・5期の棒 3.5KB と軽く、印刷にも強い | 実装時の判断（P12-1） |
| 2026-09-21 | レポートを**アプリ内の `sandbox=""` iframe に `srcdoc`** で表示する（URL を渡さない） | `data/reports/` は pywebview の内蔵サーバーからも `dev_server.py` からも配信されない（配信ルートが `web/` 固定）。また中身に AI の出力が入るので、スクリプトも同一オリジンも許可しない | 実装時の判断（P12-3・調査） |
| 2026-09-21 | **レポートの株価チャートだけ JS を許し、Lightweight Charts を HTML に丸ごとインライン埋め込みする**（上の「図はインライン SVG」の方針を株価チャートに限って撤回。他の図は SVG のまま） | ユーザーの指摘「SVG の株価チャートは視認性が悪い。ダッシュボードのトレーディングビューの UI を使ってほしい」。`srcdoc` で表示するので相対パスの外部ファイルは読めず、CDN も使わない方針なので、同梱の 198KB を埋め込むしかない。データは `<script type="application/json">` に入れてエスケープし、**AI の出力はスクリプト文脈に入れない**。JS が動かないときのために SVG 折れ線をフォールバックとして残す | ユーザー要望（P12-5） |
| 2026-09-21 | **ローソク足の陽線・陰線だけ `--chart-N` を使わず専用の `--candle-up` / `--candle-down` にする**（ライト・印刷は濃い赤と青、ダークはダッシュボードと同じ赤/青） | 実データで確認したら、Okabe–Ito の vermilion（陽線）が同じ図に重なる SMA5（orange）と見分けづらかった。ダークモードの `--chart-6` はさらにオレンジ寄りで悪化する。ローソク足は図の主役なので、ここだけは慣習どおりの赤/青を優先する | 実装時の判断（P12-6・実データレビュー） |
| 2026-09-21 | プレビューの iframe を **`sandbox="allow-scripts"`** にする（`allow-same-origin` は付けない）。白い枠・余白・背景も外す | チャートを動かすためにスクリプトだけ許す。同一オリジンを許さない限り、レポートから親ページや API には触れない（両方を同時に付けるとサンドボックスが実質無効になる）。枠はレポート自身がダークに追従するため白く浮いていた | ユーザー要望（P12-5） |
| 2026-09-21 | KPI の増減に**色を付けない**（▲▼の記号だけ）。比率の増減は**ポイント差**で出す | 増加が良いとは限らない（投資CF の流出増など）。赤＝上昇という株価の慣習と赤＝悪化という財務の慣習が衝突する。率の変化を率で出すと誤読を招く（ROE 0.150→0.343 が「▲128.7%」になっていた） | 実装時の判断（P12-2・実データレビュー） |
| 2026-09-21 | 財務数値は**「主要な経営指標等の推移」（標準要素32種・5期分）だけ**を取り込み、本表（`jppfs_cor`/`jpigp_cor`）からの科目取得は初期リリースから外す | 実データ2件で、売上・利益・CF・自己資本・EPS に加えて**発行体が算出した ROE・自己資本比率・PER まで標準要素で取れる**と確認できた。本表に降りると会計基準ごとの要素差と企業拡張要素（1書類あたり数百件）への対応が要り、得られるものに対して負担が大きい | 実装時の判断（P11 設計・ユーザー承認） |
| 2026-09-21 | 連結と単体の判別は `連結・個別` 列ではなく**コンテキストID**で行う（接尾辞なし＝連結、`_NonConsolidatedMember`＝単体、その他の `Member` は捨てる） | 実データで、連結・単体のどちらの行も `連結・個別` が `その他` になり区別できなかった。セグメント内訳も同じ要素IDで並ぶため、コンテキストIDで絞らないと値が重複する | 実装時の判断（P11 設計・実データ確認） |
| 2026-09-21 | 指標の計算は**すべてアプリ側**で行い、AI には結果だけを1指標1行で渡す | LLM は表の中の数値同士を突き合わせた計算の信頼性が低く、計算を外部に委譲するほうが精度が上がるという査読結果がある（PAL, ICML 2023）。「LLM に財務諸表を渡せばアナリストを超える」の根拠としてよく引かれた論文は**2025-02 に著者自身が撤回している** | 実装時の判断（P11 設計・調査） |
| 2026-09-21 | 財務指標を足すのと**同時にテクニカルの生 CSV を圧縮**し、プロンプト全体のトークンを増やさない | 長い入力ほど精度が落ちることが報告されており（Lost in the Middle・Context Rot）、実測で120日分が約53,000トークンに達していた。直近20日は生、それ以前はシグナル発生日と要約統計に置き換える | ユーザー（推奨案を承認） |
| 2026-09-21 | 合成スコア（Piotroski F-Score・Altman Z-Score・Beneish M-Score 等）は初期リリースで作らない | 係数が米国データで推定されたもので、日本基準での再検証を示す一次文献を確認できなかった。採用するなら「参考値」と明記する必要がある | 実装時の判断（P11 設計・調査） |
| 2026-09-21 | 会社予想に対する進捗率・達成率は**作らない**。「会社予想との比較は対象外」とレポートに明示する | 会社予想は TDnet にしかなく本ツールでは取得できない。埋め合わせようとせず、参照範囲を構造的に明示するほうが安全（調査班の一致した見解でもある） | 実装時の判断（P11 設計） |
| 2026-09-21 | **通期（FY）と中間期（HY）を `period_type` で分けて持ち、指標の算出は通期の系列だけで行う**。中間期は最新1期と前年同期だけを別枠で渡す | 半期報告書の `type=5` には、中間期の6か月の数値と前事業年度の12か月の数値が同居している（実データで確認）。同じ `revenue` として1本の系列に並べると前期比が「半年 ÷ 1年」になり、成長率が丸ごと嘘になる | 実装時の判断（P11-1 実データ確認） |
| 2026-09-21 | レポートの財務図から**営業利益・営業利益率をやめ、営業CF・フリーキャッシュフローに差し替えた** | P12-2 は「営業利益が取れる」前提で図を設計していたが、EDINET の「主要な経営指標等の推移」に**営業利益の項目は無い**（日本基準は経常利益、IFRS は税引前利益）。取れないものを軸にした図は永久に空になる | 実装時の判断（P11-6 実データ確認） |
| 2026-09-21 | レポートの増減表記から **▲▼ を廃し、`↑ ↓ →` と符号付きの数値**にした | 実データでレポートを描画して気づいた。日本語の財務資料では「▲1,000」が**マイナス**1,000を意味するので、増加の印に ▲ を付けると符号が逆に読まれる（`売上高 ▲5.5%` が減少に見える）。色に頼らず記号を添える方針自体は変えていない | 実装時の判断（P11-7 実データ確認） |
| 2026-09-21 | 確認ダイアログとレポートタブの「送信されるデータ」に**財務指標を明記**した | 実機で分析を実行したときに気づいた。プロンプトに財務指標を足したのに、同意を取る一覧が「銘柄情報・株価とテクニカル指標・開示」のままだった。**この一覧がユーザーの同意の範囲**なので、送るものを増やしたら必ず一緒に更新する（`app/ai/analyze.py` の `_SEND_KINDS` にその旨を書いた） | 実装時の判断（P7-2 実機確認） |
| 2026-09-21 | 「レポートに需給の語を一切出さない」検査を、**参照範囲の明示（`data_scope_note`）の1文を除いてから**行うようにした | SPEC §2.9.9 は `data_scope_note` に「需給データは分析に含まない」と書かせることを要求している。つまり完成したレポートには「需給」の語が必ず載る。守りたいのは需給の**値**が出ないことで、免責の文言まで禁じると仕様と矛盾する（番兵値による検査は従来どおり残している） | 実装時の判断（P11-6 レビュー） |
| 2026-09-21 | **`dps`・`payout_ratio`・`shares_outstanding`・`capital_stock` の4項目だけは、連結を採用した書類でも `_NonConsolidatedMember` から補う** | 実データで、この4項目が「提出会社の状況」の表にしか無く、連結側のコンテキストには**存在しない**と分かった（IFRS・日本基準とも）。1株当たり配当額・発行済株式総数・資本金は会社そのものの数字で、連結と単体で意味が変わらないため、補っても誤解を生まない | 実装時の判断（P11-1 実データ確認） |
| 2026-09-21 | 指標の算出を `app/financial_metrics.py` に分け、`app/financials.py`（保存・取得ジョブ）と別モジュールにした | SPEC §5 では `financials.py` 1本の予定だったが、保存層と指標の算出は依存が無く、サブエージェントに並行で書かせるときに同じファイルを取り合う（CLAUDE.md の並行作業の注意）。指標側は DB に触らない純関数にできるので、分けたほうがテストも書きやすい | 実装時の判断（P11 実装） |
| 2026-09-21 | **比率（`%` 単位）の前期比は変化率ではなくポイント差で出し、比率と倍率には CAGR を出さない** | ROE 2% → 8% を「前期比 +300%」と書くと必ず誤読される。トレンド判定をポイント差で行うと決めた以上、表に出す数字も同じ尺度にそろえないと、判定と数字が食い違って見える | 実装時の判断（P11-4 レビュー） |
