...
| 財務指標 | §2.9.6 で**アプリ側が算出した指標**（値・前期比・トレンドの3値）。会計基準・連結/単体・対象期を添える。**生の財務諸表は送らない** |
| 前提の注記 | 「開示は EDINET の法定開示のみで、決算短信・業績修正は含まれない。一覧が空でも材料が無いことを意味しない」／「会社予想との比較・同業比較は対象外」／財務数値と株価の期ずれ |
...
class AnalysisReport(BaseModel):
    ...
    fundamental: SectionAnalysis   # P11 で追加。財務数値が無い銘柄では「未取得」と書かせる
    ...
    summary: str               # 3行以内の総括
    data_scope_note: str       # P11 で追加。参照したデータの範囲（§2.9.9）
```
...
- CSS は `<style>` にインライン。**外部ファイルも CDN も参照しない**（単一ファイルで完結させる）
- **JS を出力してよいのは株価チャートのためだけ**（P12-5 で方針変更）。折りたたみは従来どおり `<details>`
  - ライブラリは同梱の `web/vendor/lightweight-charts.standalone.production.js` を**そのままインラインに埋め込む**
    （`srcdoc` で表示するので相対パスの外部ファイルは読めない。先頭の Apache-2.0 ライセンス表記ごと埋め込む）
  - データは `<script type="application/json">` に入れて `JSON.parse` する。埋め込む前に
    `<` `>` `&` と U+2028/2029 を `\uXXXX` にエスケープする（`</script>` で閉じさせない）
  - **AI の出力をスクリプト文脈に入れない。** チャートに渡すのは株価・指標・シグナル・開示日だけで、
    AI が書いた文章は従来どおり Jinja2 の autoescape 経由で HTML 本文にだけ出す
  - JS が動かない場合（スクリプトを許可しない iframe など）に備え、**チャートの中身は SVG 折れ線を初期表示にしておき、
    初期化に成功したときだけ差し替える**
- **株価チャート以外の図はインライン SVG**（`app/ai/charts.py`）
- 表示順（結論 → 根拠 → 詳細データ。P12 で改訂）:
  1. ヘッダ（銘柄情報・対象期間・生成日時・モデル名）
  2. 総括と判定（`summary` / `verdict` / `confidence`。自己申告である旨の注記つき）
  3. **株価チャート**（ダッシュボードと同じ Lightweight Charts のローソク足＋移動平均＋出来高に、
     売買シグナルと開示のマーカーを重ねる。P12-5 で SVG 折れ線から差し替え）
  4. **財務ハイライト**（KPI カード＝値・前期比・5期スパークライン／業績5期推移／CF 5期推移／
     比率の推移／PER の過去レンジと現在値。会計基準と連結・単体と対象期を明記する）
  5. テクニカルの観点（冒頭に**機械的な指標判定の集計**バッジ。AI の総合判定とは別である旨を添える）
  6. 開示の観点 → リスク → 注目点
  7. 指標値の詳細（カテゴリ分けし、主要なものだけ初期表示。残りは `<details>` で折りたたむ）
  8. 期間内のシグナル → 開示一覧 → 免責
...
- **色覚多様性に配慮した Okabe–Ito パレット**を使い、**良化・悪化を色だけで示さない**（▲▼の記号を必ず併記する）
- **増減に色を付けない。** 増加が良いとは限らない（投資キャッシュフローの流出増など）うえ、
  赤＝上昇という株価チャートの慣習と、赤＝悪化という財務の慣習が衝突するため
- **比率の指標（ROE・自己資本比率・利益率）の増減はポイント差で示す**（率の変化を率で出すと誤読を招く）
- **棒グラフの数値軸は 0 から始める。** 3D・円グラフを使わない
- 値が全期間で無い系列は、棒・線だけでなく**凡例からも外す**
- **ローソク足の陽線・陰線だけは `--chart-N` を使わず、専用の `--candle-up` / `--candle-down` を使う**
  （Okabe–Ito の vermilion はオレンジ寄りで、同じ図に重なる SMA5 と見分けがつかなくなるため。P12-5 の実機確認）。
  ライトと印刷は濃い赤と青、ダークはダッシュボード（`web/js/chart.js`）と同じ赤上げ・青下げにする
- 色は CSS カスタムプロパティ（`--chart-1`〜`--chart-8`）越しに出し、
  **ダークモード（`prefers-color-scheme`）と印刷（`@media print`）で上書きできる**ようにする。印刷時は常に白背景
- 和文フォントは OS 標準のみ（外部フォントを参照しない）。数値のセルは右揃え＋等幅数字

##### アプリ内ビューア（P12）

- レポートタブは**一覧とプレビューの2ペイン**。一覧の項目を選ぶとその場で読める。
  分析が終わったら、できたてのレポートを自動でプレビューに出す
- **`data/reports/` は pywebview の内蔵サーバーからも `dev_server.py` からも配信されない**（どちらも配信ルートが `web/` 固定）。
  そのため `get_report_html(report_id)` で HTML 文字列を受け取り、**`iframe.srcdoc` に JS のプロパティ経由で代入**する
- **iframe には `sandbox="allow-scripts"` を付ける**（スクリプトだけ許可し、**同一オリジンは許可しない**）。
  株価チャートを描くために必要。`allow-same-origin` を一緒に付けるとサンドボックスが実質無効になるので足さない。
  同一オリジンを切ると中身の高さを測れないので、**固定の高さ（70vh 目安）＋内部スクロール**にする
- プレビューの iframe に**白い枠・余白・背景を付けない。** レポート自身が `prefers-color-scheme` に追従して
  ダークでも描画されるため、枠を付けると UI に白い縁取りが出る（P12-5 のフィードバック）
- サンドボックス内からは印刷できないので、**印刷は「ブラウザで開く」に委ねる**。「保存フォルダを開く」は補助
...
### 2.9 財務数値の取り込みと指標（P11。2026-09-21 追加）

AI に多角的な分析をさせるために、有価証券報告書・半期報告書から**財務数値**を取り込む。
**生の財務諸表を AI に送ることはしない。** アプリ側で指標に落とし、圧縮した形だけをプロンプトに載せる（§2.9.6・§2.9.7）。

#### 2.9.1 取得元と判定

- EDINET の書類取得 API `documents/<docID>?type=5`（**XBRL を CSV 化したもの**）を使う。
  XBRL の生ファイル（`type=1`）を自前で解釈しない
- 提供の有無は**日次キャッシュに既にある `csvFlag`**（`"1"` なら提供あり）で判定する。**確認のための追加リクエストは不要**
...
**`連結・個別` 列で連結と単体を区別してはならない。** 実測では「主要な経営指標等の推移」の連結・単体の
どちらの行も `連結・個別` が `その他` になり、区別がつかなかった。判定は**コンテキストID**で行う。
...
LLM は表の中の数値同士を突き合わせた計算を苦手とする（`docs/research/llm-prompting.md` §1）。
**計算はすべてアプリ側で行い、AI には結果だけを渡す。**

| 指標 | 出典・算出 |
|---|---|
| ROE・自己資本比率・PER・配当性向 | **開示値をそのまま** |
| 売上高（収益）成長率 | 前期比、および取得できた期間の CAGR |
| 利益成長率 | 当期純利益（親会社株主帰属）の前期比・CAGR |
| EPS 成長率 | 前期比・CAGR |
| 営業CFマージン | 営業CF ÷ 売上高 |
...
- 合成スコア（Piotroski F-Score・Altman Z-Score・Beneish M-Score 等）は**初期リリースでは作らない**。
  係数が米国データで推定されたもので、日本基準での再検証を確認できていないため（`docs/research/financial-metrics.md`）

#### 2.9.7 AI への渡し方

- **1指標1行**の形にする（指標名・値・前期比・トレンドの3値）。生の財務諸表も5期分の全数値も送らない
- 会計基準（日本基準／IFRS／米国基準）と**連結か単体か**を必ず添える。基準をまたいだ比較をさせない
- 期ずれを明示する（「この財務数値は ○年○月期のもので、株価は直近」）。
  **決算発表から有報提出まで1.5〜2.5か月のずれがある**こと、四半期報告書は2024年に廃止されて
  **EDINET からは年2回（有報・半期報）しか取れない**ことを注記する
- 財務数値が1件も無い銘柄では、財務のセクションごと省き「財務数値は未取得」と明記する（空欄を並べない）

#### 2.9.8 テクニカル指標の圧縮（P11 と同時に行う）

財務指標を足すぶん、**プロンプト全体のトークンはむしろ減らす**。
長い入力ほど精度が落ちることが報告されており（Lost in the Middle・Context Rot。`docs/research/llm-prompting.md` §3）、
実測で 120日分は約53,000トークンに達していて、既に劣化が懸念される領域にある。

- **直近20日は生の CSV**（日々の値）を維持する
...
§2.7.4 のスキーマに次を足す。**「根拠 → 結論」の順は崩さない。**

- `fundamental: SectionAnalysis` — 財務の観点。`technical` と `disclosure` の間に置く
- `data_scope_note: str` — 参照したデータの範囲の明示。
  「会社予想との比較は対象外」「同業他社との比較は対象外」「需給データは分析に含まない」を必ず含める
...
-- 財務数値（有報・半期報の「主要な経営指標等の推移」。SPEC §2.9）
...
| `get_report_html(report_id)` | — | レポートの HTML 文字列（アプリ内ビューア用。**表示側はサンドボックス iframe 必須**） |
| `open_report(report_id)` | — | OS既定アプリで開く |
| `open_reports_folder()` | — | レポートの保存フォルダを開く |
...
└── ai/                  [新]
    ...
    ├── charts.py          レポートの図（インライン SVG。依存を増やさない）
    ├── price_chart.py     レポートの株価チャート（Lightweight Charts へ渡す JSON と同梱素材）
    └── report.py          Jinja2 で単一HTML生成
templates/               [新]
├── report.html.j2
└── report_chart.js        レポートに埋め込むチャート初期化スクリプト（テンプレートは通さない）
web/
...
|---|---|---|
| 財務諸表**本表**からの科目取得 | `jppfs_cor` / `jpigp_cor` から営業利益・棚卸資産・有利子負債・設備投資などを取り、指標を増やす | 会計基準ごとに要素 ID が異なり、企業拡張要素（実測で1書類あたり数百件）への対応が要る。まず §2.9 の「主要な経営指標等の推移」だけで運用してから判断する |
| 合成スコア（Piotroski F-Score・Altman Z-Score 等） | 複数の財務指標をまとめた1つのスコアを出す | 係数が米国データで推定されており、日本基準での再検証を示す一次文献を確認できていない。採用するなら「参考値」と明記する（`docs/research/financial-metrics.md`） |
