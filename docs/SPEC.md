...
- CSS は `<style>` にインライン。外部ファイル参照を作らない。**`<script>` を出力しない**
  （アプリ内ビューアが `sandbox=""` の iframe で表示するため。折りたたみは `<details>` で実現する）
- **図はすべてインライン SVG**（`app/ai/charts.py`）。JS のチャートライブラリも CDN も使わない
- 表示順（結論 → 根拠 → 詳細データ。P12 で改訂）:
  1. ヘッダ（銘柄情報・対象期間・生成日時・モデル名）
  2. 総括と判定（`summary` / `verdict` / `confidence`。自己申告である旨の注記つき）
  3. **株価チャート**（終値＋移動平均に、売買シグナルと開示のマーカーを重ねる）
...
- **iframe には `sandbox=""` を付ける**（スクリプトも同一オリジンも許可しない）。中身に AI の出力が入るため。
