...
| DB・キャッシュのサイズ | 登録20銘柄・3年分で DB は 50MB 以内。`edinet_cache` は gzip で1年あたり数十MB を目安とし、P4-6 で実測して本書に記す |
...
| 8 | `edinet_cache` の実サイズ | 1か月分を取得して実測（**EDINET の Subscription-Key 入手待ち**） | PLAN P4-6 |
| 9 | 後から不開示（`disclosureStatus` ≠ 0）になった書類の扱い | 再走査ではメタデータが空で返るため捨てられ、登録済みの行が残る。実データで頻度を見てから、`doc_id` ごと削除するか表示で注記するかを決める。**必須項目の欠落を一律に「削除」と解釈すると、EDINET の仕様変更で登録済みの開示を全消ししかねない**ので、いまは捨てるだけにしてある | PLAN P4-4 |
