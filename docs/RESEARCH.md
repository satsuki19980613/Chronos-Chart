...
>
> 本文はレビュー前のまま残してある。**以下の記述は誤りまたは不足と確認された。実装の根拠には [SPEC.md](SPEC.md) を使うこと。**
> 詳細と根拠 URL は [REVIEW_RESULT.md](REVIEW_RESULT.md)。
>
> | 箇所 | 本文の記述 | 正しくは |
> |---|---|---|
> | §1.3 | `secCode` で銘柄と突き合わせる | `secCode` は**提出者**の証券コード。大量保有は `issuerEdinetCode`、公開買付は `subjectEdinetCode`。EDINET コードで突合する（SPEC §2.4.3） |
> | §1.4・§1.5 | 四半期報告書（140/150）を正規化対象に挙げている | **2024年4月1日以後に開始する四半期から廃止**。今後は有報と半期報のみ |
> | §3.3 | 日証金は「ファイル名に日付が入るため未取得日だけを取れる」 | `zandaka.csv` は**固定名・最新1申込日分のみ**・Shift_JIS 系・約36列・速報/確報の区分あり（SPEC §2.3） |
> | §4.1 | 設計方針に「送信対象は…**需給数値**のみ」 | 需給は**送らない**（ユーザー決定。SPEC §2.7.3） |
> | §4.3 | 429 に RPM 超過と RPD 超過を区別するフィールドは無い | `QuotaFailure.violations[].quotaId` と `RetryInfo.retryDelay` で区別できる（非保証。SPEC §2.7.2） |
> | §4.3 | SDK は最大4回の自動リトライを組み込みで行う | `google-genai` は**既定では再試行しない**。記載の値は `HttpRetryOptions` を明示設定したときの既定値 |
> | §4.4 | 「結論 → 根拠」の順に定義すると質が上がる | 出典なし。根拠が後付けになるため**「根拠 → 結論」**にする（SPEC §2.7.4） |
> | §4.4 | `MAX_TOKENS` で切れたら分割生成へ | 出力上限と thinking 予算で対処する。分割生成はしない（SPEC §2.7.5） |
> | §5.1 | NOTICE の著作権年は 2025 | 同梱の v5.2.1 は `Copyright (c) 2026`。配布元の NOTICE 実物を同梱する |
> | §5.3 | `lineType: 2`（階段状）。whitespace data で時間軸を保つ | **`2` は曲線。階段は `LWC.LineType.WithSteps`（=1）**。whitespace は不要（SPEC §2.5.2） |
> | §5.4 | マーカーにクリックのヒットテスト API は無い | マーカーに `id` を付ければ `param.hoveredObjectId` で取れる（SPEC §2.6） |
> | §6 | 未確認事項の一覧 | 上記はいずれも一覧に無かった未確認事項である |
